...
let max_hp = 100, current_hp = 100;
let max_mp = 50, current_mp = 50;
...
// Global variable to store the canvas element
let cnv;

function setup() {
  // Increase canvas width to accommodate max controls on right (bars remain at original x)
  ...
  maxStaminaInput = createInput("100", "number").position(360, 110).size(50, 20);
...
// Enable fullscreen mode only when the canvas itself is tapped.
function mousePressed(event) {
  // Only trigger if the event's target is the canvas element.
  if (event.target === cnv.elt) {
    if (!fullscreen()) {
      fullscreen(true);
    }
  }
  // Do not return false here so that DOM elements (like buttons) remain interactive.
}

// Only prevent default behavior when touching the canvas.
